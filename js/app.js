...
$(document).ready(function () {
  ...
  $('#addMarkersBtn').on('click', function () {
    ...
    selectedDevices.forEach(function (device_id) {
...
function fetchGeofenceAndDevices(selectedDevices) {
  ...
  if (Array.isArray(selectedDevices) && selectedDevices.length > 0) {
    selectedDevices.forEach(function (device_id) {
      //deviceId = "100";
      var deviceRef = database.ref('gpsData/' + device_id);
        ...
        .then(function (snapshot) {
          ...
          if (deviceData) {
            ...
            if (dataParts.length >= 3) {
              ...
              console.log('Invalid device data format for ' + device_id);
            }
          } else {
            console.log('Device data not available for ' + device_id);
          }
...
$(document).ready(function () {
  ...
  $('#addMarkersBtn').on('click', function () {
    ...
    selectedDevices.forEach(function (deviceId) {
      ...
      if (device && device.latitude && device.longitude) {
        // Assuming device_id is a key in gpsData
        var gpsDataRef = database.ref('gpsData/' + deviceId);

        gpsDataRef.once('value')
          .then(function (snapshot) {
            var gpsData = snapshot.val();

            if (gpsData) {
              var dataParts = gpsData.split(',');
              var latitude = parseFloat(dataParts[1].split(':')[1]);
              var longitude = parseFloat(dataParts[2].split(':')[1]);

              // Add marker to the map
              var deviceMarker = L.marker([latitude, longitude])
                .bindPopup(`<b>Device ID:</b> ${deviceId}<br><b>Location:</b> ${latitude}, ${longitude}`);
              devicesLayer.addLayer(deviceMarker);
            } else {
              console.log('GPS data not available for ' + deviceId);
            }
          })
          .catch(function (error) {
  ...
  if (deviceData) {
    Object.values(deviceData).forEach(function (device) {
      if (device && device.device_id) {
        ...
        var listItem = $('<option>').val(device.fname).text(device.fname);
        var listItem = $('<option>').val(device.lname).text(device.lname);
